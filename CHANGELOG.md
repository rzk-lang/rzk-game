...
- The prelude panel lists what a `#data` declaration generates. A `#data Void` brings `rec-Void` and `ind-Void` into scope without writing them down; those names and their types now appear under the prelude source as "Also in scope", read back from rzk rather than reconstructed.
